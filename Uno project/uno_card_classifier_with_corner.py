...
Input assumption
----------------
The input to classify(...) can be either:
1. a crop containing one full UNO card, or
2. a partial card crop where at least one corner is clearly visible.

The classifier first tries the normal full-card path. If the result is weak or if
mode="corner" is requested, it uses corner-symbol templates built from the
reference images. Crops can be BGR OpenCV images, RGB images, grayscale images,
or paths to images.
"""
...
CORNER_TEMPLATE_SIZE = (96, 96)   # width, height
...
class UnoCardClassifier:
    ...
    def __init__(
        ...
        self.corner_templates = self._build_corner_templates_from_reference()
    ...
    def classify(
        ...
        """
        Classify one UNO crop.

        Parameters
        ----------
        cropped_card:
            Full-card crop or partial crop with a visible corner.
        mode:
            "full"   -> force the normal full-card classifier.
            "corner" -> force the corner-only classifier.
            "auto"   -> try full-card first, then fall back to corner if weak.

        Returns examples:
            r_5, b_skip, y_reverse, g_+2, wild, wild_draw4
        """
        crop = load_bgr(cropped_card, input_color=input_color)

        mode = mode.lower()
        if mode not in {"auto", "full", "corner"}:
            raise ValueError("mode must be 'auto', 'full', or 'corner'.")

        full_card = normalize_card_crop(crop)

        if mode == "full":
            result = self._classify_uno_card(full_card)
            result = dict(result)
            result["mode"] = "full"
            result["card"] = full_card

        elif mode == "corner":
            result = self._classify_corner_crop(crop)
            result = dict(result)
            result["mode"] = "corner"

        else:
            looks_partial = self._looks_like_partial_crop(crop)

            # If the crop shape strongly suggests a partial/corner crop, do not
            # waste time forcing the full-card classifier.
            if looks_partial:
                result = dict(self._classify_corner_crop(crop))
                result["mode"] = "corner"
            else:
                full_result = self._classify_uno_card(full_card)
                full_score = float(full_result.get("score", 0.0))

                # Most proper full-card crops are handled here. This avoids the
                # expensive corner fallback on every normal card.
                if full_score >= 0.55:
                    result = dict(full_result)
                    result["mode"] = "full"
                    result["card"] = full_card
                else:
                    corner_result = self._classify_corner_crop(crop)
                    corner_score = float(corner_result.get("score", 0.0))

                    if corner_score >= 0.35:
                        result = dict(corner_result)
                        result["mode"] = "corner"
                        result["full_candidate"] = full_result
                    else:
                        result = dict(full_result)
                        result["mode"] = "full"
                        result["card"] = full_card
                        result["corner_candidate"] = corner_result
    ...
    def classify_many(
        ...
        """Classify a list of UNO crops."""
        return [
            self.classify(card, input_color=input_color, return_details=return_details, mode=mode)
            for card in cropped_cards
        ]
    ...
    # ------------------------------------------------------------------
    # Corner-crop fallback logic
    # ------------------------------------------------------------------

    def _corner_symbol_binary(self, patch: np.ndarray) -> np.ndarray:
        """
        Binary feature for a visible UNO corner.

        This is intentionally edge-based because corner symbols are small and
        can be white text on colored background, black outline on white, or
        colored symbol on black cards.
        """
        if patch.ndim == 2:
            patch_bgr = cv2.cvtColor(patch, cv2.COLOR_GRAY2BGR)
        else:
        ...
        # Add very dark and very bright strokes, useful for white corner labels.
        dark = (v < 115).astype(np.uint8) * 255
        white = ((s < 90) & (v > 135)).astype(np.uint8) * 255
        mask = cv2.bitwise_or(edges, cv2.bitwise_or(dark, white))
        ...
        mask = cv2.resize(mask, CORNER_TEMPLATE_SIZE, interpolation=cv2.INTER_NEAREST)
        return (mask > 0).astype(np.uint8) * 255

    def _infer_reference_label_from_position(
        self,
        image_key: str,
        card: np.ndarray,
        ...
        """
        Infer the true label of a reference card from its known reference-image layout.

        This avoids using the full classifier while building corner templates, so
        initialization stays fast and the corner fallback is not circular.
        """
        img_h, img_w = image_shape[:2]
        cx, cy = center

        color, _ = self._classify_card_color(card)
        ...
        if image_key == "7_8_9":
            if color == "black":
                return self._classify_black_special(card)["label"]
            if cy < 0.36 * img_h:
                symbol = "9"
            elif cy < 0.63 * img_h:
                symbol = "8"
        ...
        raise ValueError(f"Unknown reference image key: {image_key}")

    def _build_corner_templates_from_reference(self) -> Dict[str, List[Dict[str, Any]]]:
        """
        Build templates for corner-only classification.

        For each detected reference card, we infer its label from the known
        layout, then extract the top-left and bottom-right corners. The
        bottom-right corner is rotated by 180 degrees so both corner examples
        have the same reading orientation.
        """
        templates: Dict[str, List[Dict[str, Any]]] = {}

        corner_rois = [
            ("top_left", (0.00, 0.00, 0.34, 0.32), 0),
            ("bottom_right", (0.66, 0.68, 1.00, 1.00), 180),
        ]

        for image_key, img in self.ref_images.items():
            detections, _ = detect_card_candidates(img)

            for det in detections:
                card = det["card"]
                full_label = self._infer_reference_label_from_position(
                    image_key=image_key,
                    card=card,
                    center=det["center"],
                ...
                templates.setdefault(full_label, [])

                for corner_name, roi, base_rotation in corner_rois:
                    corner = crop_card_roi(card, roi)

                    if base_rotation == 180:
                        corner = cv2.rotate(corner, cv2.ROTATE_180)

                    base = self._corner_symbol_binary(corner)

                    for angle in [0, 90, 180, 270, -12, 12]:
                        variant = rotate_patch_keep_size(base, angle)
                        templates[full_label].append(
                            {
                                "image": variant,
                                "angle": angle,
                                "source": f"{image_key}:{corner_name}",
                            }
                        )

        if not templates:
            raise RuntimeError("Could not build any corner templates from reference images.")

        return templates

    def _looks_like_partial_crop(self, crop: np.ndarray) -> bool:
        """
        Heuristic: a full card crop usually has aspect ratio near 1.5 in either
        portrait or landscape. A half-card / corner crop often deviates from that.
        """
        h, w = crop.shape[:2]
        if h <= 0 or w <= 0:
            return True
        ...
        # Very square or very elongated crops are usually not clean full cards.
        if aspect < 1.20 or aspect > 2.25:
            return True

        # If a large part of the crop is missing colored/white card material,
        # the full-card classifier is less trustworthy.
        return False

    def _classify_corner_color(self, crop: np.ndarray) -> Tuple[str, Dict[str, Any]]:
        """Estimate color from a partial crop/corner crop."""
        hsv = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
        h, s, v = cv2.split(hsv)

        black_ratio = float((v < 75).mean())
        if black_ratio > self.black_threshold:
            return "black", {"black_ratio": black_ratio, "counts": {}}
        ...
        return max(counts, key=counts.get), {"black_ratio": black_ratio, "counts": counts}

    def _candidate_corner_patches(self, crop: np.ndarray) -> List[Tuple[str, np.ndarray]]:
        """
        Generate several possible visible-corner patches from a partial crop.

        If teammates give a tight corner crop, the whole crop works. If they give
        a half card, one of these sub-crops usually contains the visible corner.
        """
        h, w = crop.shape[:2]
        patches: List[Tuple[str, np.ndarray]] = []

        patches.append(("whole", crop))

        rois = {
            "tl": (0.00, 0.00, 0.55, 0.55),
            "tr": (0.45, 0.00, 1.00, 0.55),
            "bl": (0.00, 0.45, 0.55, 1.00),
            "br": (0.45, 0.45, 1.00, 1.00),
        ...
        for name, roi in rois.items():
            patch = crop_relative(crop, roi)
            if patch.size > 0 and patch.shape[0] >= 15 and patch.shape[1] >= 15:
                patches.append((name, patch))

        return patches

    def _classify_corner_crop(self, crop: np.ndarray, top_k: int = 8) -> Dict[str, Any]:
        """
        Classify a partial UNO crop using visible corner templates.

        This is a fallback path. It is less informative than the full-card path,
        but it works when the big central symbol is missing and a corner is visible.
        """
        color, color_info = self._classify_corner_color(crop)

        scores: List[Tuple[float, str, str, float, str]] = []

        for patch_name, patch in self._candidate_corner_patches(crop):
            # Try all four orientations of the input patch because the visible
            # corner may be top-left, top-right, bottom-left, or bottom-right.
            for input_angle in [0, 90, 180, 270]:
                if input_angle == 0:
                    patch_rot = patch
                elif input_angle == 90:
                    patch_rot = cv2.rotate(patch, cv2.ROTATE_90_CLOCKWISE)
                elif input_angle == 180:
                    patch_rot = cv2.rotate(patch, cv2.ROTATE_180)
                else:
                    patch_rot = cv2.rotate(patch, cv2.ROTATE_90_COUNTERCLOCKWISE)

                query = self._corner_symbol_binary(patch_rot)

                for label, variants in self.corner_templates.items():
                    # If color is known, do not compare red crop to blue templates,
                    # except for black wild cards.
                    if color in {"r", "y", "g", "b"} and not label.startswith(f"{color}_"):
                        continue
                    if color == "black" and not label.startswith("wild"):
                        continue

                    for variant in variants:
                        score = cv2.matchTemplate(query, variant["image"], cv2.TM_CCOEFF_NORMED)[0, 0]
                        scores.append((float(score), label, patch_name, float(input_angle), variant["source"]))

        # Fallback: if color filtering eliminated everything, try all labels.
        if not scores:
            for patch_name, patch in self._candidate_corner_patches(crop):
                query = self._corner_symbol_binary(patch)
                for label, variants in self.corner_templates.items():
                    for variant in variants:
                        score = cv2.matchTemplate(query, variant["image"], cv2.TM_CCOEFF_NORMED)[0, 0]
                        scores.append((float(score), label, patch_name, 0.0, variant["source"]))

        scores = sorted(scores, reverse=True)
        best_score, best_label, best_patch, best_input_angle, best_source = scores[0]

        if best_label.startswith("wild"):
            kind = "black_special_corner"
            symbol = best_label
        else:
            kind = "corner"
            symbol = best_label.split("_", 1)[1]
        ...
        return {
            ...
            "best_input_angle": best_input_angle,
            "best_template_source": best_source,
        }
...
# -----------------------------------------------------------------------------
# Convenience function with a cache, useful if teammates just want one function.
# -----------------------------------------------------------------------------
